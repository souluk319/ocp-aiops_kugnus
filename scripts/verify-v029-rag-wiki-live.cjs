...
const wikiDocsView = readFile('komsco-ai-console-plugin/src/portal/WikiDocsView.tsx');
const wikiSource = [portalApp, wikiDocsView].join('\n');
...
assert(wikiSource.includes('fetchRagUploadedDocuments'), 'WikiDocsView must load live Gateway RAG documents');
assert(wikiSource.includes('ragUploadedDocumentToKnowledgeDoc'), 'WikiDocsView must adapt Gateway RAG documents to visible document cards');
assert(wikiSource.includes('ragListDocuments(ragUploads)'), 'WikiDocsView must support Gateway document list payloads');
assert(wikiSource.includes('usingSampleDocs'), 'WikiDocsView must explicitly track sample fallback state');
assert(wikiSource.includes('Gateway 문서 없음, 샘플 표시'), 'Wiki hero must name the sample fallback when Gateway has no documents');
assert(wikiSource.includes('searchRagDocuments(normalized)'), 'Wiki search test must call live Gateway RAG search');
assert(wikiSource.includes('ragSearchResultToKnowledgeDoc'), 'Wiki search must adapt Gateway search result chunks for display');
assert(wikiSource.includes('uploadRagDocumentFile(file)'), 'Wiki upload drawer must send selected files to Gateway RAG ingestion');
assert(wikiSource.includes("result.spec.status === 'persisted' ? '색인됨' : '검증 필요'"), 'Wiki upload queue must distinguish persisted uploads from non-persisted validation results');
assert(wikiSource.includes("doc.dataSource === 'gateway' ? 'Gateway RAG' : '샘플'"), 'Document cards must disclose Gateway vs sample source');
assert(wikiSource.includes('backendStatus') && wikiSource.includes('backendReason'), 'Wiki index drawer must expose Gateway backend status and reason');
