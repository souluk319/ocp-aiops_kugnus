...
import { severityLabel, StatusBadge } from './portalBadges';
import { formatTime } from './portalModel';
import { Panel } from './portalPanel';
import { actionRecords } from './portalLedger';
import {
  reportHealthLabel,
  reportPrimarySignal,
...
import type { AiopsRuntimeStatus, ClusterSummary, QueueItem, Severity } from './types';
...
const clusterLabel = (summary: ClusterSummary): string =>
  summary.apiUrl ? displayApiEndpoint(summary.apiUrl) : 'OpenShift 상태 확인 필요';

const displayOpenShiftVersion = (version?: string): string =>
  version ? version.replace(/-local\b/i, '') : '-';
...
export const ReportsView: React.FC<{ issueOptions: QueueItem[]; status: AiopsRuntimeStatus; summary: ClusterSummary }> = ({ issueOptions, status, summary }) => {
