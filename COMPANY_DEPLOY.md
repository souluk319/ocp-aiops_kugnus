...
Kugnus AIops를 회사 OKD/OCP OperatorHub 카탈로그에 등록할 때는 이 문서만 본다.

## 원리

`task kugnus:company:*`는 OpenShift 공식 명령이 아니라 이 repo의 바로가기다.

현재 기본 배포 패키지 버전은 `0.1.6`이다. 그래서 생성되는 CSV는 `komsco-aiops-kugnus-operator.v0.1.6`이어야 한다.
...
`Taskfile.yml` 안에 `kugnus:company:publish`라는 이름으로 실행 블록을 적어둔 것이다.
...
실제 위치는 `Taskfile.yml`의 `kugnus:company:publish` 항목이다.
...
KOMSCO_AIOPS_APPROVE_PUBLISH=komsco-ai-kugnus \
...
- `openshift-marketplace/komsco-aiops-catalog-kugnus` CatalogSource를 갱신한다.
- OperatorHub에서 `komsco-aiops-kugnus`가 검색되게 만든다.
